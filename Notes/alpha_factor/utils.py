...
def compute_forward_returns(factor,
    ...
    for period in sorted(periods):
        ...
        # added this condition for single period case mode[0] will give error
        if np.unique(days_diffs).shape[0]==1:
...
def get_clean_factor(factor,
    ...
    # modified this to drop extra date column
    merged_data['factor_quantile'] = quantile_data.reset_index(level=0, drop=True)
